'''
此程序只为了提取多个人的数据的cls维度是 [6，28546，1，318]
...
if __name__ == '__main__':
    ...
    查看和显示nii文件
    '''
    mask_imgback = nib.load('/root/autodl-tmp/LANGUAGE_test/ADHD200_mask_152_4mm.nii.gz')
    # 这是把四维nii数据转成二维数据的代码
    masker = NiftiMasker(mask_img=mask_imgback)
    masker.fit()
    inputdata2d = np.load('/root/autodl-tmp/LANGUAGE_test/data/LANGUAGE_sub_10.npy')
    ...
    np.save("/root/autodl-tmp/LANGUAGE_test/result/sub_10/normalinput2d.npy", normalinput2d)
    ...
    list_test2 = normalinput2d.reshape(316*28546).tolist()
    ...
    for j in range(57):
        ...
        np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls' + str(j) +'.npy',enc_self_attn11[:,:,0,:])
        print(enc_self_attn11.shape)
    ...
    np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls57.npy',enc_self_attn11[:,:,0,:])
    print(enc_self_attn11.shape)
...
'''
读取已经训练好的数据，并且将其综合为一个数据28546,6,318
'''
...
enc_self_attn_final = np.load('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls' + str(0) +'.npy')
for j in tqdm(range(57)):
    enc_self_attn0 = np.load('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls' + str(j+1) +'.npy')
    enc_self_attn_final = np.concatenate((enc_self_attn_final, enc_self_attn0), axis=0)
    del enc_self_attn0
av_data = enc_self_attn_final.reshape(28546 * 6, 318)
np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/2d_data_318.npy', av_data)
print(av_data.shape)
av_data = av_data[:, 1:317]
np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/2d_data_316.npy', av_data)
print(av_data.shape)
for i in range(6):
    out = moving_average(enc_self_attn_final[0,i,1:317], 10)
    out = moving_average(out, 10)
    out = moving_average(out, 10)
    out = out.reshape(1, -1)
    for j in tqdm(range(28545)):
        y_av = moving_average(enc_self_attn_final[j+1,i,1:317], 10)
    ...
    print(out.shape)
    np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head'+str(i)+'.npy', out)
    del out,y_av
np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls_final.npy', enc_self_attn_final)
import numpy as np
file_A = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/2d_data_316.npy'
file_0 = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head0.npy'
file_1 = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head1.npy'
file_2 = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head2.npy'
file_3 = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head3.npy'
file_4 = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head4.npy'
file_5 = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/out_av_head5.npy'
file_B = '/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls_final.npy'
...
print(head_av.shape)
np.save('/root/autodl-tmp/LANGUAGE_test/result/sub_10/head_av.npy',head_av)
import scipy.io as io

io.savemat('/root/autodl-tmp/LANGUAGE_test/result/sub_10/2d_data_316.mat', {'data232': A})
io.savemat('/root/autodl-tmp/LANGUAGE_test/result/sub_10/head_avLANGUAGE.mat', {'head_avLANGUAGE': head_av})

#删除部分中间结果
import os
print('删除部分中间结果')
for de in tqdm(range(58)):
    if os.path.exists('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls' + str(de) +'.npy'):
        os.remove('/root/autodl-tmp/LANGUAGE_test/result/sub_10/enc_self_attn_cls' + str(de) +'.npy')
    else:
